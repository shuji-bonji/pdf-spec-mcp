...
export class CompareService {
  private fullComparePromise: Promise<FullCompareResult> | null = null;

  constructor(private indexProvider: { getSectionIndex(specId: string): Promise<SectionIndex> }) {}

  /**
   * Compare sections between PDF 1.7 and PDF 2.0.
   * @param section - Optional PDF 2.0 section number to filter results
   */
  async compareVersions(section?: string): Promise<CompareVersionsResult> {
    // Build or reuse cached full comparison
    if (!this.fullComparePromise) {
      logger.info('CompareService', 'Building version comparison (first call)...');
      this.fullComparePromise = this.buildFullComparison();
    }

    const full = await this.fullComparePromise;
   ...
   */
  private async buildFullComparison(): Promise<FullCompareResult> {
    const start = Date.now();

    // Load both section indexes
    const [index17, index20] = await Promise.all([
      this.indexProvider.getSectionIndex('pdf17'),
      this.indexProvider.getSectionIndex('iso32000-2'),
    ]);
...
export const defaultCompareService = new CompareService({ getSectionIndex });
 ...
 * @param section - Optional PDF 2.0 section number to filter results
 */
export const compareVersions = (section?: string) => defaultCompareService.compareVersions(section);
