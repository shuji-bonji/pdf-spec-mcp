...
**own-content（分割片）と公開応答は別物**（SV-1 / 2026-07-19）: 分割片はセクションごとに
互いに素で、requirements / search 索引はこれから作る。公開の get_section / get_tables は
**children リンクの DFS でサブツリーを文書順に連結**して返す（親 = 前文 + 全子孫。
Table 257 のように子にしか無い規範が親経由で見えないのは実害だった）。
連結が重複を生まないのは分割片が互いに素だから — **索引側を誤って公開側
（getSectionContent）に繋ぐと祖先の数だけ二重計上される**。索引は必ず
getOwnSectionContent を使うこと。
なお子孫の解決に文字列 prefix を使ってはいけない（Annex の子はキーが
「A.1 General」のようなタイトル全体で、prefix では届かない）。
