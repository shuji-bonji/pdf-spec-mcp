...
- ~~**#6（起動時キャッシュ DB）は今回のスコープ外**~~ → ✅ **0.5.0 として公開（2026-08-25）・npx 2 プロセス検証 PASS・Issue #6 クローズ済み**。
  方針は Issue 原案から変えた（起動時一括 → 初回利用時に構築して JSON 保存、SQLite は使わない、
  Docker プリロードは著作権上やらない）。計画と Phase 0 の実測は `docs/issue6-index-store-plan.md`、
  落とし穴は CLAUDE.md の 7。公開版検証（サンドボックス・MCP stdio・2 プロセス）: search 23,007 → 99 ms、要件全走査 50,423 → 142 ms、6 呼び出しとも本文同一
