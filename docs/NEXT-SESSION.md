...
- ~~**S-1. ページ跨ぎの表の抽出漏れ**（🔴）~~ — ✅ **完了（2026-07-18）**。
  ...
  全 988 セクションの走査で **148 個の表**が行を取りこぼしていたことが判明し、すべて回復。
  旧実装との差分で行を失った表 0 件・表の個数が変わった表 0 件を確認済み
...
- **S-4. ヘッダなしの表は継続行を回復できない**（🟡）— `collectStructTreeTables` の連結条件が
  `headers.length > 0` のため、ヘッダ行を持たない表（例: 8.7.4.5.5 の Coons patch フラグ表）は
  連結されない。継続側だけ足すと**連結されずに表が 1 個増え、以降の `table_index` がずれる**ため、
  B-S1 では意図的に対象外にした（実際に一度その退行を出して気づいた）。
  位置情報や列数での同一性判定が要る
- **S-5. 同じ「帯」が get_section / get_requirements でも落ちている**（🔴）— B-S1 は
  `get_tables` に限定して直した。しかし取り残される帯は表に限らず、**本文・要件文も同様に
  どのセクションにも属していない**（Table 182 の QuadPoints 行の "shall" は get_requirements からも
  見えない）。`getSectionContent` 側で帯を前セクションへ寄せれば全ツールが一度に直り、
  次セクション側は `trimToSectionStart` が捨てているので**二重計上は起きない**。
  ただし get_section の出力が広範に変わるため、requirements インデックスと e2e baseline への
  影響を測ってから入れること
...
4. **S-5（帯を get_section 側で直す）** — B-S1 の一般化。**← 次はここを推奨**
   （表以外の本文・要件文も同じ理由で落ちているため）
5. **A-4（McpServer + zod）+ A-5（構造化エラー）** — 構造の刷新（外部仕様は不変）
6. **B-S2（コーパス明示）+ S-4（ヘッダなしの表）**

## 2026-07-18 セッションの記録（A-1〜A-3 + B-S1）

- **未リリース**。CHANGELOG は `[Unreleased]` に積んである。A-4/A-5 まで進めてから
  まとめて 1 回で版を上げるのがよい（規約 §2.8「空 bump を避ける」の精神）。
  ただし B-S1 は 148 個の表の欠落を直す**利用者影響の大きい修正**なので、
  A-4 を待たず先に minor を切る判断もありうる
- B-S1 の検証手法が有効だったので次回も使うこと: **旧実装を `git worktree` で並べてビルドし、
  全 988 セクションに `get_tables` を流して JSON 差分を取る**。これで
  「行を失った表 0 / 表の個数が変わった表 0」を機械的に示せた。
  実際この差分が 8.7.4.5.5 の退行（ヘッダなし表で表が 1 個増える）を捕まえた —
  実 PDF での全数差分がなければ見落としていた
- ユニットテストは `src/services/pdf-service.test.ts`。`PDFSpecService` は registry / loader を
  コンストラクタ注入するので、**vi.mock なしで合成 PDF を組んで実コードを通せる**。
  新しいテストは**必ず旧実装に当てて落ちることを確認**すること（今回 2 件が落ちるのを確認済み）
