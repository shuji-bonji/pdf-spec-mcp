...
- **S-7. 要件抽出が表を見ていない**（🔴 新規・2026-07-18）— `extractRequirementsFromContent` は
  paragraph / list / note しか走査せず、**表のセルを完全に無視している**。ISO の表は
  「(Required) ... shall be Highlight ...」のように要件語の宝庫で、実測では
  **先頭 260 セクションの表 106 個中、"shall" を含むセルが 301 個**あり、すべて
  `get_requirements` から不可視。Table 182 の QuadPoints 行だけでも shall が 3 つある。
  正典としては S-5 より影響が大きい可能性がある。
  設計課題: セル単位で要件を切り出すと文脈（どのキーの話か）が落ちるため、
  `Requirement` に表・行のキーを持たせるか検討が要る
...
5. **S-7（要件抽出が表を見ていない）** — **← 次はここを推奨**。正典としての影響が大きい
6. **A-4（McpServer + zod）+ A-5（構造化エラー）** — 構造の刷新（外部仕様は不変）
7. **B-S2（コーパス明示）+ S-4（ヘッダなしの表の分裂）**
